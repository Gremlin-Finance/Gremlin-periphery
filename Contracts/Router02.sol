...
contract Router02 is IRouter02, IGremlinCallee {
    ...
    modifier ensure(uint256 deadline) {
        require(deadline >= block.timestamp, "GremlinRouter: EXPIRED");
        _;
    }
    ...
    function deleverage(
        ...
        require(redeemTokens > 0, "GremlinRouter: REDEEM_ZERO");
    ...
    function _removeLiqAndRepay(
        ...
        .burn(address(this));
        require(amountAMax >= amountAMin, "GremlinRouter: INSUFFICIENT_A_AMOUNT");
        require(amountBMax >= amountBMin, "GremlinRouter: INSUFFICIENT_B_AMOUNT");
        // repay and refund
    ...
    function gremlinBorrow(
        ...
        require(sender == address(this), "GremlinRouter: SENDER_NOT_ROUTER");
        require(
            msg.sender == declaredCaller,
            "GremlinRouter: UNAUTHORIZED_CALLER"
        );
    ...
    function _optimalLiquidity(
        ...
        if (amountBOptimal <= amountBDesired) {
            require(
                ...
                "GremlinRouter: INSUFFICIENT_B_AMOUNT"
            );
            ...
            require(
                ...
                "GremlinRouter: INSUFFICIENT_A_AMOUNT"
            );
    ...
    function _quote(
        ...
        require(amountA > 0, "GremlinRouter: INSUFFICIENT_AMOUNT");
        require(
            reserveA > 0 && reserveB > 0,
            "GremlinRouter: INSUFFICIENT_LIQUIDITY"
        );
    ...
    {
        require(index < 2, "GremlinRouter: INDEX_TOO_HIGH");
        address _borrowable;
        borrowable = address(
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"9e06ef8c91c4f8554c7f19a1b99adb7840967e8674ba9d4b50fd20561f8a38ed" // Borrowable bytecode keccak256
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"420f34a44c9c9b7772eb781a3683fd3f7e7803fc4061d2e15576cb6c8aafd7ce" // Collateral bytecode keccak256
